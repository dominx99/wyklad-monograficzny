...
document.querySelector('#submit-button').addEventListener('click', (e) => {
    ...
    fetch('/lista2/zadanie4/oblicz', {
        method: 'POST',
        body: JSON.stringify({
            values: input.value.split(' ').map(Number),
            mergeBoundaries: document.getElementById('merge-boundaries').checked,
...
function setResult(data) {
    document.getElementById('result-result_list').innerHTML = data.result_list;
    document.getElementById('result-p_value_table').innerHTML = data.p_value_table;
...
function renderHistogramChart(data) {
    ...
    console.log(binLabels, binValues);
